 ...
 *
*/
@implementation SHMinusOperator
...
- (id)init
{	
	//logInfo(@"SHMinusOperator.m: initing ok");
	if(!initError){
...
{	
	...
	// logInfo(@"SHMinusOperator.m: Added inputs, number of inputs is %i", [self numberOfInputs] );
...
- (BOOL)execute:(id)fp8 head:()np time:(double)timeKey arguments:(id)fp20
{
	...
	//		logInfo(@"SHMinusOperator.m: Evaluate.. result is.. %@", resultWrapper );
	//		_dirtyBit = NO;
		}
		_evaluationInProgress = NO;
	} else {
		logWarning(@"SHMinusOperator.m: Wont Evaluate - dirty bit not set" );
	}
	return YES;
}
