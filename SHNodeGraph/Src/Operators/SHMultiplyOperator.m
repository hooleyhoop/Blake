...
@implementation SHMultiplyOperator
...
- (id)init
{	
	//logInfo(@"SHMultiplyOperator.m: initing ok");
	if(!initError){
...
{	
	...
	// logInfo(@"SHMultiplyOperator.m: Added inputs, number of inputs is %i", [self numberOfInputs] );
...
- (BOOL)execute:(id)fp8 head:()np time:(double)timeKey arguments:(id)fp20
{
	...
	//		logInfo(@"SHMultiplyOperator.m: Evaluate.. result is.. %@", resultWrapper );
	//		_dirtyBit = NO;
		}
		_evaluationInProgress = NO;
	} else {
		logWarning(@"SHMultiplyOperator.m: Wont Evaluate - dirty bit not set" );
	}
	return YES;
}
