...
- (void) awakeFromNib
{
	// NSLog(@"SHSwapableView: awakeFromNib");
...
// - randomPoint
// ===========================================================
// returns a random point inside the bounds of the view
- (NSPoint)randomPoint
{
    NSPoint result;
//  NSRect r; // NSPoint and NSRect are C structs
  int width, height;
//  r = [self bounds];	
	// NB! HERE
	width = 200;
	height = 200;
	GLfloat px = (random() % width)-width/2;
	GLfloat py = (random() % height)-height/2;	
	
//	NSLog(@"width is %f. random is %f", [_camera frustumWidth], px);
