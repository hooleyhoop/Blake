...
+(NSString*)applicationLogName {
	// NB - Don't use [NSBundle executablePath], or kCFBundleNameKey here.  When you launch a command line tool from inside a real app, [NSBundle mainBundle] returns the original app's bundle, which isn't helpful
	// Fortunately, processName seems to work just as well:
...
//steve	nsenumerat(crashes, crash) {
		// Scan through all the crash files, picking out the ones that start with our app name
//steve		if ([crash rangeOfString:[self applicationLogName]].location==0)
//steve			[ourCrashes addObject:[crashDir stringByAppendingPathComponent:crash]];
//steve	}
	...
	if (self = [super init]) {
		...
		if (![mgr createDirectoryAtPath:[BBLogFile logDir] withIntermediateDirectories:YES attributes:nil error:&errorInfo]) {
			...
			if ([mgr fileExistsAtPath:oldestLogFile] && ![mgr removeFileAtPath:oldestLogFile handler:nil])
				logError(@"Couldn't drop oldest log file\n");
			...
			if (!logFileHandle)
				logError(@"Couldn't create output file handle at %s", [logFile fileSystemRepresentation]);
