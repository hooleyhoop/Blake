...
{
	return [OCMockObject _makeNice:[self mockForClass:aClass]];
}

+ (id)niceMockForProtocol:(Protocol *)aProtocol
{
	return [OCMockObject _makeNice:[self mockForProtocol:aProtocol]];
}
...
- (void)forwardInvocation:(NSInvocation *)anInvocation
{
	...
	{
		NSString *invDesc = [anInvocation invocationDescription];
		NSString *recDesc = [self _recorderDescriptions:NO];
		NSException *exception = [NSException exceptionWithName:NSInternalInconsistencyException reason:
			[NSString stringWithFormat:@"%@: unexpected method invoked: %@ %@",  [self description], invDesc, recDesc] userInfo:nil];
