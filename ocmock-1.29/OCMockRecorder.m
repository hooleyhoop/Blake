...
- (BOOL)matchesInvocation:(NSInvocation *)anInvocation
{
	...
	NSString *sel1 = NSStringFromSelector([anInvocation selector] );
	NSString *sel2 = NSStringFromSelector([recordedInvocation selector]);
	...
	for(i = 2; i < n; i++)
	{
		...
		if([passedArg isProxy] && [recordedArg isProxy])
			if(passedArg!=recordedArg)
				return NO;
			else
				return YES;
		...
		else
		{
			...
			if(([recordedArg class] == [NSNumber class]) && ([(NSNumber*)recordedArg compare:(NSNumber*)passedArg] != NSOrderedSame))
				return NO;
			if(([recordedArg class] == [NSString class]) && ([(NSString*)recordedArg isEqualToString:(NSString*)passedArg]==NO)){
				return NO;
			}
			else if(([recordedArg isEqual:passedArg] == NO) && !((recordedArg == nil) && (passedArg == nil)))
				return NO;
		}
	}
	return YES;
